...
from craftax_ma.game_logic import calculate_light_level
from craftax_ma.util.maths_utils import get_all_players_distance_map
from craftax_ma.craftax_state import EnvState, Inventory, Mobs
from craftax_ma.util.game_logic_utils import get_ladder_positions
from craftax_ma.util.noise import generate_fractal_noise_2d
from craftax_ma.world_gen.world_gen_configs import (
    ALL_DUNGEON_CONFIGS,
...
def generate_dungeon(rng, static_params, config):
    ...
    def _add_room(carry, room_index):
        ...
        block_map = block_map.at[
            room_position[0] + chest_position[:, 0],
            room_position[1] + chest_position[:, 1],
...
def generate_world(rng, params, static_params):
    ...
    mob_projectiles, mob_projectile_directions, mob_projectile_owners = (
        _create_projectiles(static_params.max_mob_projectiles)
    )
    ...
    growing_plants_positions = jnp.zeros(
        (static_params.max_growing_plants * static_params.player_count, 2),
        dtype=jnp.int32,
    )
    ...
    state = EnvState(
        ...
        chests_opened=jnp.zeros(
            (static_params.num_levels, static_params.player_count), dtype=bool
        ),
